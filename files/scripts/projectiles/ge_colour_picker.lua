...
if ( comps ~= nil ) then
	for i,v in ipairs( comps ) do
        ...
        if ( ComponentGetValue2( v, "value_bool" ) ~= nil ) then
            isHex = ComponentGetValue2( v, "value_bool" )
        end
	end
end
...
if ( colour ~= nil ) then
	...
	if ( comps ~= nil ) then
		for i,v in ipairs( comps ) do
			...
			if cosmetic then
                if (isHex) then
                    ComponentSetValue2( v, "emitted_material_name", "lava") -- Glows
                    ComponentSetValue2( v, "color", colour)
                elseif ( particle ~= nil ) then
